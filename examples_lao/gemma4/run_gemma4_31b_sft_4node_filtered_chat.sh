...
# Launch Gemma4 31B SFT (filtered_chat, 4-node TP4) in a Kubernetes environment.
#
# Expected env vars injected by the platform:
#   MASTER_ADDR
#   MASTER_PORT
#   RANK          (0-based node rank)
#   WORLD_SIZE    (total number of nodes, should be 4)
#
# Run this script inside every worker pod:
#   bash examples_lao/gemma4/run_gemma4_31b_sft_4node_filtered_chat.sh

set -euo pipefail

REPO_ROOT=${REPO_ROOT:-/llm-align/liuchonghan/Automodel_lao}
CONFIG=${CONFIG:-examples_lao/gemma4/gemma4_31b_sft_4node_tp4_filtered_chat.yaml}
...
if [ "${WORLD_SIZE}" -ne 4 ]; then
    echo "Warning: WORLD_SIZE=${WORLD_SIZE}, expected 4 nodes for this config." >&2
fi
...
mkdir -p \
    ...
    "${WANDB_DIR}" \
    "${WANDB_CACHE_DIR}" \
    /llm-align/liuchonghan/ckpt_automodel/gemma4_31b_sft_4node_tp4_filtered_chat
